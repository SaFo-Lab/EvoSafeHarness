# Prior Art — Securing Tool-Using Agents Against Prompt Injection
...
| 3 | **Separate control flow from data flow.** Untrusted data may fill in *values* but must not choose *actions*. | Approximate the legitimate action-scope; gate effect-bearing calls that the data (not the user) introduced. |
| 4 | **Least privilege; narrow the blast radius first.** | Risk-tier tools (read < write < exec); scrutinize the dangerous ones harder in the pre-tool gate. |
