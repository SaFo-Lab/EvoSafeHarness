...
# Load .env into THIS process's environment BEFORE reading DTAP_MODEL below.
# cascade.py is invoked directly (not via run_dtap.sh, which sources .env), so
# without this a bare `python scripts/cascade.py` silently fell back to the hard
# default model. setdefault → never override a value already set in the shell.
if ENV_FILE.is_file():
